...
public class MemberPortalMapper {
	...
	@Autowired
	public MemberPortalMapper(final MemberPortalService memberPortalService,
			final PortalService portalService,final MemberService memberService) {
	...
	public Mbr mapUpdateMemberStatusRequest2Mbr(UpdateMemberStatus body) throws Exception{
		...
		try{
			...
			if(AttributetobeUpdatedEnum.MemberStatus.toString().equalsIgnoreCase(body.getAttributetobeUpdated().toString())){
			...
			if(AttributetobeUpdatedEnum.MemberPortalEligibilityFailDt.toString().equalsIgnoreCase(body.getAttributetobeUpdated().toString())){
	...
	public MbrPrtl mapaddMemberAttrRequest2MbrPrtl(AddMemberAttrRequest body) throws Exception{
		LOGGER.info(Thread.currentThread().getStackTrace()[1].getMethodName()+" :: START");
		MbrPrtl mbrPrtl = null;DateFormat dateFormatter = new SimpleDateFormat("yyyy-MM-dd");
		if(body!=null){
			mbrPrtl=new MbrPrtl();
	 ...
	 */
	public GetMemberPortalAttrResponse mapMbrPrtl2GetMemberPortalAttrResponse(List<MbrPrtl> list) throws ParseException, GlobalNavException{
		...
		if(!CollectionUtils.isEmpty(list)){
			...
			for (MbrPrtl mbrPrtl : list) {
				...
				if(mbrPrtl.getMbrByMbrId()!=null){
					...
					getMemberAttrResponse.setProvisionMemberId(new Integer(mbrPrtl.getMbrByMbrId().getPrvsnMbrId()).toString());
					...
					if(mbrPrtl.getMbrByMbrId().getMdmMbrDob()!=null && isNotBlank(mbrPrtl.getMbrByMbrId().getMdmMbrDob().toString()))
	...
	public List<GetMemberAttrResponse> mapMbrPrtl2GetMemberAttrResponse(List<MbrPrtl> list) throws ParseException, GlobalNavException{
...
//		//change columnname to new parameter
//		if(body.getnewColum()!=null) {
//			mbr.setNewColumnName(body.getnewColumn());
//		}
		//CAGM Fields for RX
